...
**Sequencing dependency (preferred):** the long-term goal is to promote the hidden-stream pattern to a first-class baseStorage primitive (modelled on Pryv's existing `isShown: false` flag for hidden account fields such as `:_system:invitationToken` / `:_system:referer`), so any plugin can opt streams out of regular API responses by configuration rather than middleware. CMC shipped before that promotion landed, so the plugin carries its own filter middleware as interim debt, to be removed once the platform-wide primitive ships and `:_cmc:_internal:*` can declare `isShown: false` declaratively.
