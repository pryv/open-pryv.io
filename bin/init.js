...
# # at once (429 over the ceiling), and access.maxRequestBytes: the stored
# # size of one request (413 over it). Creating a request needs no
...
async function main () {
  ...
  yamlBody += '#\n# Prefer pure-ENV configuration (no config file mounted)? Convert this\n';
  yamlBody += '# file with `./config-to-env.sh` — it writes a sibling env file (one\n';
  yamlBody += '# KEY=VALUE per config key, nested paths joined with __) consumable by\n';
  yamlBody += '# `docker run --env-file <file>.env pryvio/open-pryv.io`.\n';
  ...
  // Converts the sibling config into an env file for pure-ENV
  // deployments (`docker run --env-file …` with no config mount).
  // Same overwrite policy as the other launchers.
  ...
  if (shouldWriteEnvScript) {
    const envScript = [
      '#!/bin/sh',
      '# Auto-generated by `pryvio/open-pryv.io init`.',
      '# Converts the sibling pryv-config.yml into pryv-config.env — one',
      '# KEY=VALUE per config key (nested paths joined with __) — so the',
      '# server can run with NO config file at all:',
      '#',
      '#   ./config-to-env.sh',
      '#   docker run --env-file pryv-config.env … ' + defaultImageRef(),
      '#',
      '# Overrides:',
      `#   PRYV_IMAGE     docker image tag (default: ${defaultImageRef()})`,
      'set -e',
