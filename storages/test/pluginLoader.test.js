...
describe('[PLUG] Plugin Loader infrastructure', () => {
  ...
  describe('pluginLoader', () => {
    ...
    it('[PLUG-RESOLVE-MISDECLARE] init() throws when config picks an engine for a storageType the engine does not declare', async () => {
      // The filesystem engine declares only fileStorage. A config that
      // picks it for platformStorage must fail fast here rather than
      // later via the PlatformDB interface validator's cryptic
      // "missing method: <X>" error.
      const fakeConfig = {
        has: (key) => key === 'storages:platform:engine',
        get: (key) => key === 'storages:platform:engine' ? 'filesystem' : null
      };
      await assert.rejects(
        async () => { await pluginLoader.init(fakeConfig); },
        /Configured engine "filesystem" for storages\.platform\.engine.*does not declare storageType "platformStorage"/
      );
    });
  });
});
