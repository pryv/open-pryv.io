...
components/       Source code split by domain
  ...
  shared-secrets/   One-time secret hand-off by key (namespace plugin, `:_shared-secrets:`)
...
- **Account delegation guards: don't relax them.** A delegate token (and any access created with one) acts on the controlled account, so the delegation plugin (`components/delegation/src/hooks.ts`) keeps a server-owned lineage: `accesses.create` stamps `clientData.delegation = { kind: 'delegated-child', relId, delegate, viaAccessId }` from the authenticated access only; a client-supplied `delegation` key is refused on create and update, and the marker is preserved across `clientData` updates. Detach deletes the delegated-child accesses of the relationship. A delegation-derived access may not create a durable grant through a path that records no lineage: OAuth2 consent accept (`routes/oauth2.ts`, `403 access_denied`), CMC `consent/accept-cmc` / `consent/scope-update-cmc` / `consent/request-cmc` (`400`, `delegation-grant-requires-owner`) and the `/reg/access` shared-secret hand-off (`routes/reg/credentialHandoff.ts`, falls back to inline delivery). Lifecycle guards exempt exactly `kind === 'delegated-child'`; don't widen that exemption, and stamp lineage on a new grant path before letting a delegate token use it.
- **A managed `shared` access never outlives its managing `app` access.** The expiry chain treats a missing expiry as "never": create gives such a shared the app's `expires`, clearing it or narrowing the app over a never-expiring child is refused (`components/api-server/src/methods/accesses.ts`), and `MethodContext.checkManagingAccessExpiry` refuses a never-expiring shared once its app expired; webhooks apply the same rule before firing (`business/src/webhooks/repository.ts` `accessIsUsable`). Resolve the managing access with `managingAccessBase(createdBy)` (`business/src/accesses/refs.ts`): an app authenticated with a caller id records `createdBy` as `<id> <callerId>`.
